...
author: "Maxence Poirier-Joanette, Adrien Badet, Alexandre Ethier, Bérince Hounsouvo"
format: 
  revealjs:
    theme: black
    transition: slide
...
![Sizerin flammé](Sizerin.jpg){height="280px" fig-align="center"}

![Tarin des pins](Tarin.jpg){height="280px" fig-align="center"}
:::

## [Objectifs]{style="color: yellow;"}
...
a)  Est-ce que la proportion de jeunes de l’année (indice de succès reproducteur) est corrélée aux irruptions ?

. . .

b)  Est-ce que la condition corporelle est corrélée aux irruptions ?

## [Jeux de données brutes]{style="color: yellow;"}
...
## [Manipulations et défis de *abond*]{style="color: yellow;"}
...
## [Manipulations et défis de *bague*]{style="color: yellow;"}
...
## [Manipulations par espèce]{style="color: yellow;"}
...
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Irruption/DUSA_irr.R")
...
[Comment évoluent les populations de fringillidés boréaux (4 espèces) dans le temps (1996-2025) ; tendances temporelles sur 30 ans de suivi?]{style="color: yellow;"}
...
[Modèle linéaire suivi d'une randomisation]{style="color: yellow;"}

## [Code pour la tendance temporelle de l'abondance]{style="color: yellow;"}
...
plot_SIFL_tot <- ggplot(abond[abond$Espece=="SIFL",], aes(x = Annee, y = abond_std, group =1))+
  ...
  labs(title = "Abondance standardisée du Sizerin flammé dans le temps",
       x = "Année",
       y = expression("N. individus recensés *" ~ heure^{-1}))+
  theme_classic()+
  theme(axis.text.x = element_text(size = 10, angle = 45, vjust = 0.8),
...
plot_JABO_tot <- ggplot(abond[abond$Espece=="JABO",], aes(x = Annee, y = abond_std, group =1))+
  ...
  labs(title = "Abondance standardisée du Jaseur boréal dans le temps",
       x = "Année",
       y = expression("N. individus recensés *" ~ heure^{-1}))+
  theme_classic()+
  theme(axis.text.x = element_text(size = 10, angle = 45, vjust = 0.8),
...
plot_TAPI_tot <- ggplot(abond[abond$Espece=="TAPI",], aes(x = Annee, y = abond_std, group =1))+
  ...
  labs(title = "Abondance standaridsée du Tarin des pins dans le temps",
       x = "Année",
       y = expression("N. individus recensés *" ~ heure^{-1}))+
  theme_classic()+
  theme(axis.text.x = element_text(size = 10, angle = 45, vjust = 0.8),
...
## [Visualisation de la tendance temporelle de l'abondance des Fringillidés]{style="color: yellow;"}
...
## [Modèle d'abondance pour DUSA]{style="color: yellow;"}
...
## [Tableau IC à 95% pour l'abondance]{style="color: yellow;"}
...
## [Code pour les estimés de log beta avec IC 95% pour l'abondance]{style="color: yellow;"}
...
## [Figure pour IC à 95% pour les modèles d'abondance]{style="color: yellow;"}
...
plot(tab_comp$annee,
     xaxt = "n",
     ylim = c(-0.2, 0.25),
     ylab = expression(paste("log estimé de ", beta, " année")),
     xlab = "Espèces",
     main = expression(paste("Estimés de log ", beta,  " avec IC 95% : abondance")))
abline(h = 0, lty = 2, col = "red")
axis(side = 1, at = axex, labels = c("DUSA", "JABO", "SIFL", "TAPI"))
segments(x0 = 1, y0 = DUSA_lowerIC_annee,
         x1 = 1, y1 = DUSA_upperIC_annee)
...
## [Randomisation pour l'abondance]{style="color: yellow;"}

```{r}
#| echo: true
#| eval: false
#| code-line-numbers: "|5|7-10|13|15-26|28"
...
## [Histogramme de randomisation des modèles d'abondance]{style="color: yellow;"}

```{r}
# save(DUSA_output, file = "DUSA_output.R")
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_rand/DUSA_output.R")
# save(JABO_output, file = "JABO_output.R")
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_rand/JABO_output.R")
# save(SIFL_output, file = "SIFL_output.R")
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_rand/SIFL_output.R")
# save(TAPI_output, file = "TAPI_output.R")
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_rand/TAPI_output.R")
...
## [Code pour la tendance temporelle de la proportion de jeunes]{style="color: yellow;"}
...
DUSA_prop<-ggplot(data = DUSA, aes(x = Annee, y = prop_HY)) +
  geom_col(fill= "darkorange") +
  scale_x_continuous(breaks = seq(2007, 2022, by = 5), limits = c(2006,2025)) +
  xlab("Année") +
  ylab("Proportion HY") +
  geom_text(aes(label = nb_tot), vjust = -0.5, size = 2)+
  labs(fill = "Proportion HY", title = "Proportion de jeunes de Durbec des sapins par année")+
...
## [Modèle de proportion pour DUSA]{style="color: yellow;"}
...
## [Code pour les estimés de log beta avec IC 95% pour la proportion]{style="color: yellow;"}
...
plot(tab_comp_prop$annee,
     ...
     ylab = expression(paste("log estimé de ", beta, " année")),
     xlab = "Espèces",
     main = expression(paste("Estimés de log ", beta,  " avec IC 95% : proportion")))
abline(h = 0, lty = 2, col = "red")
axis(side = 1, at = axex, labels = c("DUSA", "JABO", "SIFL", "TAPI"))
...
## [Visualisation des estimés de log beta avec IC 95% pour la proportion]{style="color: yellow;"}
...
## [Randomisation pour la proportion]{style="color: yellow;"}

```{r}
#| echo: true
#| eval: false
#| code-line-numbers: "|3|5-8|10|12-23|27"
# simulation
...
pbeta_annee <- sum(DUSA_output_prop[, 2] <= coef(lm_DUSA_prop)[2])/n_sim # 0.28
```

## [Histogramme de randomisation des modèles de proportion]{style="color: yellow;"}

```{r}

#Amène tous les jeux de données
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_rand/DUSA_output_prop.R")
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_rand/JABO_output_prop.R")
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_rand/SIFL_output_prop.R")
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_rand/TAPI_output_prop.R")

#Amène les modèles linéaires
lm_DUSA_prop <- lm(prop_HY ~Annee, weights = nb_tot, data = DUSA)
lm_JABO_prop <- lm(prop_HY ~Annee, weights = nb_tot, data = JABO)
lm_SIFL_prop <- lm(prop_HY ~ Annee, weights = nb_tot, data = SIFL)
lm_TAPI_prop <- lm(prop_HY ~Annee, weights = nb_tot, data = TAPI)
...
## [Code pour la tendance temporelle de la condition]{style="color: yellow;"}
...
[L'abondance ne change pas pour aucune espèce à Tadoussac au fil du temps]{style="color: yellow;"}

. . .

[La proportion de jeunes le Jaseur boréal diminue dans le temps à Tadoussac, mais augmente pour le Tarin des pins]{style="color: yellow;"}

. . .

[La condition est régie par des processus morphologiques qui évoluent peu dans le temps]{style="color: yellow;"}

## [Objectif #2]{style="color: yellow;"}

[Qu'est-ce qui explique les irruptions des différentes espèces de fringillidés?]{style="color: yellow;"}
...
[Régression bêta-binomiale à l'aide de JAGS]{style="color: yellow;"}
...
Les irruptions d'oiseaux sont définies par le mouvement semi-périodique (migration facultative) d'un grand nombre d'individus en dehors de leur aire de résidence habituelle. Ces mouvements à grande échelle sont moins prévisibles que les migrations saisonnières et s'apparentent au déplacement d'individus en fonction de conditions environnementales favorables ou défavorables (Widick et al., 2023).
...
## [Code pour visualiser les irruptions]{style="color: yellow;"}
...
#DUSA_plot
DUSA_plot <- ggplot(DUSA, aes(x = Annee, y = abond_std, color = factor(irruption)))+
...
SIFL_plot <- ggplot(SIFL, aes(x = Annee, y = abond_std, color = factor(irruption)))+
...
TAPI_plot <- ggplot(TAPI, aes(x = Annee, y = abond_std, color = factor(irruption)))+
...
## [Régression bêta-binomiale JAGS]{style="color: yellow;"}

<br>

$S_i \sim Binomiale(p_i, n_i)$
...
$logit(\mu_{_i}) = \alpha_{annee} + \beta_{irru}*irru{_i}$
...
#| echo: true
#| eval: true
#| code-line-numbers: "|3-13|15-17|19-30|32-46|56-58|59-66|70-73|76|78-85|88"
...
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_JAGS/DUSA_out_jags.txt")
```
...
## [Diagrammes de convergence]{style="color: yellow;"}
...
## [Code des diagrammes d'autocorrélation]{style="color: yellow;"}
...
## [Distributions postérieures]{style="color: yellow;"}
...
## [Extraction des coefficients du JAGS]{style="color: yellow;"}
...
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_JAGS/DUSA_out_jags.txt")
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_JAGS/JABO_out_jags.txt")
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_JAGS/TAPI_out_jags.txt")
load("/Users/maxencepoirier-joanette/Rstudio/FOR7046/Fringilids/Data/Output_JAGS/SIFL_out_jags.txt")
...
## [Code pour visualiser l'effet des irruptions par espèce]{style="color: yellow;"}
...
## [Effets des irruptions par espèce sur la proportion]{style="color: yellow;"}
...
plot(axex, tab_comp_JAGS$mean,
      xaxt = "n",
      ylim = range(c(tab_comp_JAGS$ic_inf, tab_comp_JAGS$ic_sup)),
      ylab = expression(paste("log estimé de ", beta, " irruption")),
      xlab = "Espèces",
      pch = 16,
      main = expression(paste("Estimés de log ", beta,  " avec IC 95% : proportion")))
...
:::
::::
## [Code pour comparer les années irruptives vs non-irruptives]{style="color: yellow;"}
...
## [Condition DUSA irruptif vs non-irruptif]{style="color: yellow;"}
...
## [Condition JABO irruptif vs non-irruptif]{style="color: yellow;"}
...
## [Condition SIFL irruptif vs non-irruptif]{style="color: yellow;"}
```{r}
boxplot(SIFL_bague$condition[SIFL_bague$SIFL_IRR=="0"],
        SIFL_bague$condition[SIFL_bague$SIFL_IRR=="1"],
        col = c("grey","turquoise3"), 
...
## [Condition TAPI]{style="color: yellow;"}
...
## [Tableau des conditions selon le stade irruptif]{style="color: yellow;"}
...
[Différences statistiques significatives, mais différences biologiques minimes]{style="color: yellow;"}
...
[Pour le JABO, la proportion de jeunes augmente lors d'années irruptives]{style="color: yellow;"}

. . .

[Pour la condition, différences marginales]{style="color: yellow;"} . . .

## [Discussion]{style="color: yellow;"}
